...
export const AICoachModal: React.FC<AICoachModalProps> = ({ session, isOpen, onClose, isGuest, profile, settings, onLoginRequest, onShareStats }) => {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
        const rawResponse = await chatWithCoach(messages.map(m => ({ role: m.role, text: m.text })), textToSend);
        
        if (rawResponse === "REACHED_LIMIT") {
            setUsage(getUsageStatus());
        } else {
            // Parse dynamic suggestions if present
            const suggestionPattern = /\[SUGGESTIONS:\s*(.*)\]/s;
            const match = rawResponse.match(suggestionPattern);
            let aiText = rawResponse;
            
            if (match) {
                aiText = rawResponse.replace(suggestionPattern, '').trim();
                try {
                    ...
                    console.debug("Suggestion parse failed");
                }
            }

            setMessages(prev => [...prev, { role: 'model', text: aiText, timestamp: time }]);
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#111827] w-full max-w-lg rounded-[2.5rem] overflow-hidden border border-slate-800 shadow-2xl flex flex-col h-[85vh] relative animate-message-pop">
        ...
        <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-900/50 shrink-0">
          ...
          <button onClick={onClose} className="w-10 h-10 rounded-xl bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center transition-all border border-slate-700 active:scale-90"><i className="fa-solid fa-xmark"></i></button>
        </div>
                ...
                <div className="flex-1 overflow-hidden relative bg-slate-950/20">
                    {activeTab === 'insight' ? (
                        ...
                        <div className="flex flex-col h-full bg-[#0a0f14]/50">
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-48 no-scrollbar scroll-smooth">
                                {messages.length === 0 && (
                                ...
                                {messages.map((m, i) => (
                                    <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-message-pop`}>
                                        <div className={`max-w-[85%] ${m.role === 'user' ? 'bg-gradient-to-tr from-brand-600 to-brand-500 text-white rounded-tr-none' : 'bg-white/5 backdrop-blur-md border border-white/10 text-slate-200 rounded-tl-none'} p-4 rounded-2xl shadow-xl relative`}>
                            ...
                            {/* MODERN FLOATING CONTROLS AREA */}
                            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0a0f14] via-[#0a0f14]/95 to-transparent pt-10">
                                {/* DYNAMIC CONTEXTUAL RECOMMENDATIONS */}
                                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-4 animate-fade-in px-1">
                                    {currentSuggestions.map((s, i) => (
                                        <button 
                                            key={i} 
                                            onClick={() => handleSend(s)} 
                                            className="bg-slate-800/80 hover:bg-brand-600 border border-slate-700 hover:border-brand-500 px-4 py-2 rounded-full text-[10px] font-black text-slate-300 hover:text-white transition-all whitespace-nowrap shadow-lg active:scale-95"
                                        >
                                            {s}
                                        </button>
                                    ))}
                                ...
                                <div className="flex gap-2 items-center">
                                    <div className="flex-1 bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-1.5 flex gap-2 shadow-2xl focus-within:border-brand-500/50 transition-all">
                                        <input 
                                            value={input}
                                            onChange={(e) => setInput(e.target.value)}
                                            onKeyDown={(e) => e.key === 'Enter' && handleSend()}
                                            placeholder="Poocho coach se..."
                                            className="flex-1 bg-transparent px-4 py-3 text-sm text-white outline-none font-bold placeholder:text-slate-700"
                                        />
                                        <button 
                                            onClick={() => handleSend()}
                                            disabled={chatLoading || !input.trim()}
                                            className="w-12 h-12 bg-brand-600 hover:bg-brand-500 disabled:opacity-30 rounded-2xl text-white shadow-xl active:scale-90 transition-all flex items-center justify-center shrink-0"
                                        >
                                ...
                                <div className="flex justify-between items-center px-2 mt-3">
                                    ...
                                    <p className="text-[8px] font-black text-brand-500/50 uppercase tracking-widest italic flex items-center gap-1">
                                        <i className="fa-solid fa-shield-halved text-[7px]"></i> Encrypted Connection
                                    </p>
                                </div>
                            </div>
                        </div>
