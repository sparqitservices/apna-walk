                ...
                {profile.isGuest ? (
                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-8 animate-fade-in">
                        <div className="relative">
                            <div className="w-24 h-24 bg-slate-800 rounded-[2rem] flex items-center justify-center text-4xl border border-slate-700 shadow-inner">
                                <i className="fa-solid fa-lock text-slate-600"></i>
                            </div>
                            <div className="absolute -top-2 -right-2 w-8 h-8 bg-brand-600 rounded-xl flex items-center justify-center text-white border-4 border-slate-900 shadow-lg animate-bounce">
                                <i className="fa-solid fa-user-plus text-xs"></i>
                            </div>
                        </div>
                        <div className="space-y-3">
                            <h3 className="text-white font-black text-2xl italic tracking-tighter uppercase">Identity Required</h3>
                            <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed">
                                Arre Boss! To find buddies and track live squads, you need to be logged in. 
                                Guests walk solo, but legends walk together!
                            </p>
                        </div>
                        <button 
                            onClick={() => { onClose(); /* Trigger login is handled in parent via onLoginRequest or similar if needed */ }} 
                            className="bg-brand-600 hover:bg-brand-500 text-white font-black py-4 px-10 rounded-[1.5rem] shadow-xl shadow-brand-500/20 active:scale-95 transition-all uppercase tracking-[4px] text-xs"
                        >
                            Sign In Now
                        </button>
                    </div>
                            ...
                            {loading && view !== 'chat' && (
                                <div className="h-full flex flex-col items-center justify-center">
                                    <div className="relative w-20 h-20 mb-8">
                                        ...
                                        <div className="w-20 h-20 border-4 border-brand-500/20 border-t-brand-500 rounded-full animate-spin shadow-xl"></div>
                                    </div>
                                    <p className="font-black uppercase text-[10px] tracking-[6px] text-brand-500 animate-pulse">Syncing Vibe Map...</p>
                                </div>
                            )}
