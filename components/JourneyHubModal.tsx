...
export const JourneyHubModal: React.FC<JourneyHubModalProps> = ({ isOpen, onClose, history, onViewSegment }) => {
    ...
    // Aggregate all points for the selected day's map
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[150] flex flex-col animate-fade-in">
            ...
            <header className="p-6 border-b border-white/5 bg-slate-900/50 flex justify-between items-center shrink-0">
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-[4px]">History Hub</p>
                </div>
                    ...
                    <div className="bg-slate-800/40 rounded-[3rem] border border-slate-700/50 overflow-hidden shadow-2xl">
                        <div className="h-64 w-full relative">
                            {allDayPoints.length > 0 ? (
                                <RouteMap route={allDayPoints} className="h-full w-full opacity-80" />
                            ) : (
                            ...
                            <div className="absolute bottom-6 left-8 right-8 flex justify-between items-end">
                                <div>
                                    <p className="text-brand-400 text-[10px] font-black uppercase tracking-[4px] mb-1">Today's Total</p>
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-slate-500 text-[8px] font-black uppercase tracking-widest">Aggregate</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 pt-4">
                        ...
                        {sortedSessions.length === 0 ? (
                            <div className="py-12 text-center text-slate-700">
                                <p className="text-[10px] font-black uppercase tracking-widest">Stay active to generate path forensics.</p>
                            </div>
                        ) : (
                                ...
                                {sortedSessions.map((session, idx) => (
                                    <div key={session.id} className="relative pl-12">
                                        ...
                                        <div 
                                            ...
                                            className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-5 hover:bg-slate-800/60 transition-all cursor-pointer active:scale-[0.98] group"
                                        >
